...
class ApiTest extends TestCase
{
    ...
    public function test_getting_artists(): void
    {
        ...
        $response->assertJson(fn (AssertableJson $json) => $json->has('data', 1)
            ->first(fn (AssertableJson $json) => $json->has('artists', 2)
                ->has('artists.0', fn (AssertableJson $json) => $json->where('name', 'Polaris')
                    ->where('category', 'listening')
                    ->has('albums', 1)->has('albums.0', fn (AssertableJson $json) => $json->where('artistName', 'Polaris')
                    ->where('album', 'The Death of Me')
                    ->etc()
                    )
                    ->etc()
                )
                ->has('artists.1', fn (AssertableJson $json) => $json->where('name', 'Depeche Mode')
                    ->where('category', 'other')
                    ->has('albums', 2)->has('albums.0', fn (AssertableJson $json) => $json->where('artistName', 'Depeche Mode')
